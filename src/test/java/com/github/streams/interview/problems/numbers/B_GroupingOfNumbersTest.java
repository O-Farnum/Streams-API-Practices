...
class B_GroupingOfNumbersTest {
  ...
  void testGroupingOfNumbers() {
    ...
    Map<Integer, List<Integer>> result = randomIntegers.stream().collect(Collectors.groupingBy(i -> i/10));
    final var mySolution = InterviewProblemSolutions.groupingOfNumbers(randomIntegers);
    var yourSolution = result;

    for(int key : yourSolution.keySet()) {
      System.out.print(key * 10 + ": ");
      yourSolution.get(key).stream().forEach(a -> System.out.print(a + " "));
      System.out.println();
    }
